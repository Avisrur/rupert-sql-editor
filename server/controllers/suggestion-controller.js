...
async function tableNamesStartsWith(req, res, next) {
  const { rows } = await talbeNamesDB.getAllTableNamesThatStartsWith(req.body.startsWith + "%");
  res.json(rows);
}
...
async function columnNames(req, res, next) {
  ...
  if (rows.length !== 0) {
    const { rows: column_rows } = await columnNamesDB.getAllColumnNamesByTableId(rows[0].id);
